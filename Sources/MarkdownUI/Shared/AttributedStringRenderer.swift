...
final class AttributedStringRenderer {
  ...
  #if !os(watchOS)
    func attributedString(for document: Document, attachments: [String: NSTextAttachment])
      -> NSAttributedString
    {
...
extension AttributedStringRenderer {
  ...
  fileprivate func attributedString(for blocks: [Block]) -> NSAttributedString {
    blocks
      .map { attributedString(for: $0) }
      .joined(separator: NSAttributedString(string: Constants.paragraphSeparator))
  }

  fileprivate func attributedString(for block: Block) -> NSAttributedString {
    switch block {
    ...
    case let .html(value):
      ...
      var documentAttributes: NSDictionary? = [:]
        ...
        let result = try? NSMutableAttributedString(
          data: data, options: options, documentAttributes: &documentAttributes)
  ...
  fileprivate func attributedString(for inlines: [Inline]) -> NSAttributedString {
    inlines
      .map { attributedString(for: $0) }
      .joined()
  }

  fileprivate func attributedString(for inline: Inline) -> NSAttributedString {
    switch inline {
    case let .text(value):
  ...
  fileprivate func attributedString(for list: List) -> NSAttributedString {
    saveState()
    defer { restoreState() }
  ...
  fileprivate func attributedString(for item: Item, delimiter: String, isLastItem: Bool)
    -> NSAttributedString
  {
...
extension List {
  fileprivate func delimiter(at index: Int) -> String {
    switch style {
    case .bullet:
      return "\u{2022}"
